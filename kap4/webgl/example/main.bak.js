...
(function() {
	...
	function initGl() {
		...
		// Recalculate if perspective parameter changes
	...
	function calcViewProjMatrix() {
		...
		if (cfg.perspective) {
			...
			mat4.perspective(projMatrix, degToRad(45), aspectRatio, 0.1, 5);
		} else {
			mat4.ortho(projMatrix, -1, 1, -1, 1, 0, 4);
		}
	...
	function initBuffers() {
		...
		setupBuffer(vertices, 'a_Position');
		setupBuffer(colors, 'a_Color');
	...
	function render() {
		...
		gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);
		drawMethode = (cfg.wireframe) ? gl.LINE_STRIP : gl.TRIANGLES;
